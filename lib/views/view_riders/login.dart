...
import 'package:hatidapp/views/view_riders/forgot.dart';
...
class LoginPageState extends State<LoginRider> {
  bool obscure = true;
              ...
              GestureDetector(
                onTap: () {
                  Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (context) => ForgotPageRider()));
                },
                child: Center(
                  child: Text(
                    "Forgot Password?",
        ...
        if (resVal[0] != 'err_no') {
          if (resVal[1] != '0') {
            ...
            Library().alertError(context,
                'This account is not yet verified by the system. Please wait for the confirmation of the admin');
          }
        } else {
          Navigator.pop(context);
  ...
  setPref(userId) async {
    ...
    setSharedPreds.setString('riderId', userId.toString());
    return "0";
  }
}
